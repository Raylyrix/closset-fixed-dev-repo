...
export class EnhancedVectorTools {
     ...
     */
    handlePathSelection(event, point, shapes) {
        // Find clicked shape
     ...
     */
    handleAddAnchor(event, point, shapes) {
    ...
    handleConvertAnchor(event, point, shapes) {
        ...
        if (hitResult.type === 'anchor' && hitResult.target.pointIndex !== undefined) {
            ...
            if (shape) {
                ...
                if (newType === 'smooth') {
                    const controlPoints = BezierCurveEngine.calculateControlPoints(updatedPoints[pointIndex - 1] || null, updatedPoints[pointIndex], updatedPoints[pointIndex + 1] || null, {
                        maxControlLength: 50,
                        minControlLength: 5,
                        smoothness: 0.8,
     ...
     */
    handlePathOperations(event, point, shapes) {
        // Find clicked shape
        const clickedShape = this.findShapeAtPoint(point, shapes);
        if (clickedShape) {
            return {
     ...
     */
    handleShapeBuilder(event, point, shapes) {
     ...
     */
    handlePenMove(event, point, currentPath) {
     ...
     */
    handleSelectionMove(event, point, shapes) {
     ...
     */
    handleCurvatureMove(event, point, shapes) {
     ...
     */
    handleShapeBuilderMove(event, point, shapes) {
     ...
     */
    handlePenUp(event, point, currentPath) {
     ...
     */
    handleSelectionUp(event, point, shapes) {
        if (this.isSelecting && this.selectionBox) {
            this.isSelecting = false;
            const selectedShapes = this.findShapesInSelectionBox(this.selectionBox, shapes);
            this.selectionBox = null;
     ...
     */
    handleCurvatureUp(event, point, shapes) {
        return {
            success: true,
     ...
     */
    handleShapeBuilderUp(event, point, shapes) {
        return {
            success: true,
    ...
    findShapeAtPoint(point, shapes) {
        return shapes.find(shape => point.x >= shape.bounds.x &&
            point.x <= shape.bounds.x + shape.bounds.width &&
            point.y >= shape.bounds.y &&
            point.y <= shape.bounds.y + shape.bounds.height) || null;
    }
    findClosestPathSegment(point, shapes) {
    ...
    findShapesInSelectionBox(selectionBox, shapes) {
        const minX = Math.min(selectionBox.start.x, selectionBox.end.x);
        const maxX = Math.max(selectionBox.start.x, selectionBox.end.x);
        const minY = Math.min(selectionBox.start.y, selectionBox.end.y);
        const maxY = Math.max(selectionBox.start.y, selectionBox.end.y);
        return shapes.filter(shape => shape.bounds.x >= minX &&
