...
import { pointInPolygon, calculateBoundingBox } from './vectorMath';
...
export function pathUnion(path1, path2) {
...
export function pathDifference(path1, path2) {
...
export function pathExclusion(path1, path2) {
...
export function pathToCurves(points, tension = 0.5) {
    if (points.length < 2)
        return [];
    const segments = [];
    for (let i = 0; i < points.length - 1; i++) {
...
function calculateControlPoint(prev, current, next, tension, direction) {
    if (!prev && !next)
        return current;
    let angle;
    let distance;
...
export function pathsOverlap(path1, path2) {
    const bbox1 = calculateBoundingBox(path1);
    const bbox2 = calculateBoundingBox(path2);
...
export function calculatePathArea(points) {
    if (points.length < 3)
        return 0;
    let area = 0;
    for (let i = 0; i < points.length; i++) {
...
export function calculatePathPerimeter(points, closed = false) {
    if (points.length < 2)
        return 0;
    let perimeter = 0;
    for (let i = 0; i < points.length - 1; i++) {
