...
export function renderSimpleStitch(ctx, points, config) {
    if (points.length < 2)
        return;
    ctx.save();
...
export function renderSimpleFill(ctx, points, config) {
    if (points.length < 3)
        return;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
...
export function renderSimpleCircle(ctx, points, config) {
    if (points.length < 1)
        return;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
...
export function renderSimpleDashed(ctx, points, config) {
    if (points.length < 2)
        return;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
...
export function renderSimpleCross(ctx, points, config) {
    if (points.length < 1)
        return;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
...
export function renderSimpleBranches(ctx, points, config) {
    if (points.length < 2)
        return;
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
...
export function renderStitchType(ctx, points, config) {
    if (!ctx || !points || points.length === 0 || !config)
        return;
    switch (config.type) {
        case 'cross-stitch':
        case 'crossstitch':
