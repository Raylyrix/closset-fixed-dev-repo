...
export function onProfileUpdate(cb) {
    subscribers.add(cb);
...
function getRefreshHz() {
    // Heuristic: measure rAF intervals for a second
    return undefined;
}
async function sampleFPS(durationMs = 750) {
...
function scoreFromRenderer(adapter) {
    if (!adapter)
        return 30;
    const s = adapter.toLowerCase();
    // Very rough heuristic keywords
    if (s.includes('rtx') || s.includes('radeon rx') || s.includes('arc'))
        return 90;
    if (s.includes('gtx') || s.includes('vega') || s.includes('m1') || s.includes('m2'))
        return 75;
    if (s.includes('uhd') || s.includes('iris') || s.includes('intel'))
        return 45;
    return 60;
}
function classify(cpuScore, gpuScore, ramGB, fps) {
...
export async function getHardwareProfile() {
    ...
    const cpuScore = await new Promise(resolve => {
    ...
    const profile = { tier, cpuScore, gpuScore, ramGB, adapter };
    emit(profile);
    return profile;
}
