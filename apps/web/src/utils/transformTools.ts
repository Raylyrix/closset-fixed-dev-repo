...
import { Point2D, BoundingBox, calculateBoundingBox } from './vectorMath';

export interface Transform {
  id: string;
  type: 'scale' | 'rotate' | 'skew' | 'perspective' | 'flip';
  center: Point2D;
...
export function createTransformHandles(bounds: BoundingBox): TransformHandle[] {
...
export function flipPointsHorizontally(
  points: Point2D[],
  center: Point2D
): Point2D[] {
  return points.map(point => ({
...
export function flipPointsVertically(
  points: Point2D[],
  center: Point2D
): Point2D[] {
  return points.map(point => ({
...
export function applyTransform(
  ...
  if (transform.params.scaleX !== undefined || transform.params.scaleY !== undefined) {
    result = scalePoints(
      result,
      transform.params.scaleX || 1,
      transform.params.scaleY || 1,
      transform.center
    );
  }
  ...
  if (transform.params.skewX !== undefined || transform.params.skewY !== undefined) {
    result = skewPoints(
      result,
      transform.params.skewX || 0,
      transform.params.skewY || 0,
      transform.center
    );
  }
...
export function calculateTransformMatrix(transform: Transform): number[] {
  ...
  return multiplyMatrices(
    multiplyMatrices(
      multiplyMatrices(translateBack, multiplyMatrices(rotation, multiplyMatrices(scale, translateToOrigin))),
      skew
    ),
    matrix
  );
...
export function applyMatrixTransform(point: Point2D, matrix: number[]): Point2D {
  return {
