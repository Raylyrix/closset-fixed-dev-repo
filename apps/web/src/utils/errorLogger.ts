...
class ErrorLogger {
  ...
  private criticalThreshold = 3; // Mark as critical after 3 occurrences
  ...
  public addFix(errorId: string, fixDescription: string): void {
    if (this.errorLogs.has(errorId)) {
  ...
  public getRecurringErrors(): ErrorLog[] {
  ...
  public getErrorPattern(patternId: string): ErrorPattern | undefined {
    return this.errorPatterns.get(patternId);
  }

  public getAllErrorPatterns(): ErrorPattern[] {
  ...
  public importLogs(data: string): void {
    try {
  ...
  private generateErrorId(errorType: string, description: string): string {
    return `${errorType}_${description.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}`;
  }

  private getPreventionSteps(errorType: string): string[] {
    const pattern = this.errorPatterns.get(errorType);
    return pattern ? pattern.preventionTips : [];
  }

  private checkForPatterns(errorType: string, description: string): void {
  ...
  private logToConsole(errorType: string, description: string, severity: string): void {
...
export const logVectorError = (description: string, affectedComponents: string[] = ['vector-tools']) => {
  errorLogger.logError('vector_tools_error', description, affectedComponents, 'high');
};

export const logRenderingError = (description: string, affectedComponents: string[] = ['rendering']) => {
  errorLogger.logError('rendering_error', description, affectedComponents, 'medium');
};

export const logStateError = (description: string, affectedComponents: string[] = ['state-management']) => {
  errorLogger.logError('state_management_error', description, affectedComponents, 'high');
};

export const logPerformanceError = (description: string, affectedComponents: string[] = ['performance']) => {
