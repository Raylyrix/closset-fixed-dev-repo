...
export function renderCrossStitch(ctx, points, config) {
...
export function renderSatinStitch(ctx, points, config) {
    // Only log in development
    if (process.env.NODE_ENV === 'development') {
...
export function renderChainStitch(ctx, points, config) {
    // Only log in development
    if (process.env.NODE_ENV === 'development') {
...
export function renderFillStitch(ctx, points, config) {
    // Only log in development
    if (process.env.NODE_ENV === 'development') {
        console.log(`🩸 RENDERING FILL STITCH with ${points.length} points`);
    }
    if (points.length < 3)
        return; // Need at least 3 points for a fill area
    // Save context state to avoid interference
    ctx.save();
    // Ensure proper rendering context
    ctx.globalCompositeOperation = 'source-over';
...
export function renderBullionStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderFeatherStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j <= numFeathers; j++) {
            ...
            ctx.lineTo(featherX + Math.cos(angle + Math.PI / 4) * featherLength, featherY + Math.sin(angle + Math.PI / 4) * featherLength);
            ctx.moveTo(featherX, featherY);
            ctx.lineTo(featherX + Math.cos(angle - Math.PI / 4) * featherLength, featherY + Math.sin(angle - Math.PI / 4) * featherLength);
            ctx.stroke();
        }
...
export function renderBackStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderFrenchKnot(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderRunningStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderBlanketStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j <= numStitches; j++) {
            ...
            ctx.lineTo(stitchX + Math.cos(angle) * stitchLength, stitchY + Math.sin(angle) * stitchLength);
            ctx.lineTo(stitchX + Math.cos(angle) * stitchLength + Math.cos(angle + Math.PI / 2) * stitchLength / 2, stitchY + Math.sin(angle) * stitchLength + Math.sin(angle + Math.PI / 2) * stitchLength / 2);
            ctx.stroke();
        }
    }
    ctx.restore();
...
export function renderHerringboneStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j <= numStitches; j++) {
            ...
            ctx.moveTo(stitchX, stitchY);
            ctx.lineTo(stitchX + Math.cos(angle) * stitchLength, stitchY + Math.sin(angle) * stitchLength);
            ctx.lineTo(stitchX + Math.cos(angle) * stitchLength + Math.cos(angle + Math.PI / 3) * stitchLength / 2, stitchY + Math.sin(angle) * stitchLength + Math.sin(angle + Math.PI / 3) * stitchLength / 2);
            ctx.stroke();
        }
    }
    ctx.restore();
...
export function renderLazyDaisyStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderCouchingStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderAppliqueStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderSeedStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j <= numSeeds; j++) {
            ...
            ctx.lineTo(seedX + Math.cos(angle) * seedLength, seedY + Math.sin(angle) * seedLength);
            ctx.stroke();
        }
    }
    ctx.restore();
...
export function renderStemStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderSplitStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderBrickStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderLongShortStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderFishboneStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j < numStitches; j++) {
            ...
            ctx.beginPath();
            ctx.moveTo(stitchX, stitchY);
            ctx.lineTo(stitchX + Math.cos(angle + Math.PI / 4) * stitchLength, stitchY + Math.sin(angle + Math.PI / 4) * stitchLength);
            ctx.moveTo(stitchX, stitchY);
            ctx.lineTo(stitchX + Math.cos(angle - Math.PI / 4) * stitchLength, stitchY + Math.sin(angle - Math.PI / 4) * stitchLength);
            ctx.stroke();
        }
...
export function renderSatinRibbonStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderMetallicStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderGlowThreadStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderVariegatedStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
...
export function renderGradientStitch(ctx, points, config) {
    if (process.env.NODE_ENV === 'development') {
