...
export class ErrorPrevention {
  private static instance: ErrorPrevention;
  private preventionRules: Map<string, () => boolean> = new Map();

  private constructor() {
    this.initializePreventionRules();
  }

  public static getInstance(): ErrorPrevention {
    ...
    this.preventionRules.set('anchor_points_vector_mode', () => {
      ...
      if (!appState.vectorMode) {
        errorLogger.logError(
          'anchor_points_vector_mode_violation',
          'Attempted to render anchor points outside vector mode',
          ['vector-tools', 'anchor-points'],
          'high'
        );
        return false;
      }
  ...
  public checkAnchorPointsRendering(): boolean {
    return this.preventionRules.get('anchor_points_vector_mode')?.() || false;
  }

  public validateCanvasContext(ctx: CanvasRenderingContext2D | null): boolean {
    if (!ctx) {
      errorLogger.logError(
        'canvas_context_null',
        'Canvas context is null when trying to render',
        ['canvas', 'rendering'],
        'critical'
      );
      return false;
  ...
  public validateVectorShape(shape: any): boolean {
    if (!shape) {
      errorLogger.logError(
        'vector_shape_null',
        'Vector shape is null or undefined',
        ['vector-tools', 'shape-rendering'],
        'high'
      );
      return false;
    }
    ...
    if (!shape.path) {
      errorLogger.logError(
        'vector_shape_no_path',
        'Vector shape has no path property',
        ['vector-tools', 'shape-rendering'],
        'high'
      );
      return false;
    ...
    if (!shape.path.points || !Array.isArray(shape.path.points)) {
      errorLogger.logError(
        'vector_shape_invalid_points',
        'Vector shape has invalid points array',
        ['vector-tools', 'shape-rendering'],
        'high'
      );
      return false;
  ...
  public validateStitchConfig(config: any): boolean {
    if (!config) {
      errorLogger.logError(
        'stitch_config_null',
        'Stitch configuration is null or undefined',
        ['stitch-rendering', 'configuration'],
        'high'
      );
      return false;
    }
    ...
    if (!config.type) {
      errorLogger.logError(
        'stitch_config_no_type',
        'Stitch configuration has no type property',
        ['stitch-rendering', 'configuration'],
        'high'
      );
      return false;
    ...
    if (!config.color) {
      errorLogger.logError(
        'stitch_config_no_color',
        'Stitch configuration has no color property',
        ['stitch-rendering', 'configuration'],
        'medium'
      );
      return false;
  ...
  public validateLayer(layer: any): boolean {
    if (!layer) {
      errorLogger.logError(
        'layer_null',
  ...
  public validateToolTransition(fromTool: string, toTool: string): boolean {
    ...
    if (fromTool === 'vector' && toTool !== 'vector') {
      ...
      if (appState.vectorMode) {
        errorLogger.logError(
          'vector_mode_not_cleaned',
          'Vector mode not properly cleaned up during tool transition',
          ['vector-tools', 'tool-transition'],
          'high'
        );
        return false;
      }
  ...
  public checkPerformanceImpact(operation: string, startTime: number): void {
    const duration = performance.now() - startTime;
    
    if (duration > 100) {
      errorLogger.logError(
        'performance_impact_high',
        `Operation '${operation}' took ${duration.toFixed(2)}ms`,
  ...
  public validateStateConsistency(): boolean {
    const appState = useApp.getState();
    ...
    if (appState.vectorMode && !appState.activeTool) {
      errorLogger.logError(
        'state_inconsistency',
        'Vector mode is active but no active tool is set',
        ['state-management', 'vector-tools'],
        'high'
      );
      return false;
    }
  ...
  public preventMemoryLeaks(): void {
    ...
    if (errorLogs.length > 10) {
      errorLogger.logError(
        ...
        'High number of recurring errors detected',
  ...
  public generatePreventionReport(): string {
