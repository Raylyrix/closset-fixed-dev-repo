...
export class EnhancedStitchRenderer {
    ...
    renderEnhancedCrossStitch(ctx, points, config) {
        if (points.length < 2)
            return;
        ctx.save();
    ...
    renderEnhancedSatinStitch(ctx, points, config) {
        if (points.length < 2)
            return;
        ctx.save();
        ctx.globalCompositeOperation = 'source-over';
    ...
    renderEnhancedChainStitch(ctx, points, config) {
        if (points.length < 2)
            return;
        ctx.save();
        ctx.globalCompositeOperation = 'source-over';
    ...
    renderEnhancedFillStitch(ctx, points, config) {
        if (points.length < 3)
            return;
        ctx.save();
        ctx.globalCompositeOperation = 'source-over';
    ...
    getThreadProperties(threadType) {
    ...
    getQualitySettings(quality) {
        ...
        return settings[quality] || settings.normal;
    }
    calculateOptimalStitchDistribution(points, config, threadProps) {
        const stitches = [];
    ...
    calculateThreadVariation(threadProps, x, y) {
    ...
    calculateStitchAngle(p1, p2, direction) {
        const baseAngle = Math.atan2(p2.y - p1.y, p2.x - p1.x) * 180 / Math.PI;
        return (baseAngle + direction) % 360;
    }
    renderSingleCrossStitch(ctx, stitch, config, threadProps, quality) {
    ...
    drawCrossStitch(ctx, x, y, size, angle) {
    ...
    renderSatinWithThreadSimulation(ctx, points, config, threadProps, quality) {
    ...
    renderChainWithLinkSimulation(ctx, points, config, threadProps, quality) {
        const baseColor = this.parseColor(config.color);
    ...
    renderChainLink(ctx, x, y, size, baseColor, threadProps) {
    ...
    renderFillWithDirectionSimulation(ctx, points, config, threadProps, quality) {
        const baseColor = this.parseColor(config.color);
    ...
    calculateColorVariation(baseColor, threadProps, x, y) {
