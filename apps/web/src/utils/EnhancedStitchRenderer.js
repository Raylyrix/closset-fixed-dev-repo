...
export class EnhancedStitchRenderer {
     ...
     */
    renderStitch(points, config, options) {
        if (points.length < 2)
            return;
        ...
        if (options.enableCaching && options.canvas) {
            const imageData = options.context.getImageData(0, 0, options.canvas.width, options.canvas.height);
     ...
     */
    setupHighQualityRendering(ctx, config, settings) {
        ctx.save();
     ...
     */
    applyThreadTypeEffects(ctx, config) {
     ...
     */
    applyTransformations(ctx, options) {
     ...
     */
    renderByType(points, config, ctx, settings) {
        switch (config.type) {
            case 'satin':
     ...
     */
    renderSatinStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
     ...
     */
    renderFillStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
     ...
     */
    renderCrossStitch(points, config, ctx, settings) {
     ...
     */
    renderOutlineStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        ctx.beginPath();
        ctx.moveTo(points[0].x, points[0].y);
     ...
     * Render chain stitch
     */
    renderChainStitch(points, config, ctx, settings) {
     ...
     * Render backstitch
     */
    renderBackstitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
     ...
     * Render French knot
     */
    renderFrenchKnot(points, config, ctx, settings) {
     ...
     */
    renderBullionStitch(points, config, ctx, settings) {
     ...
     */
    renderLazyDaisy(points, config, ctx, settings) {
     ...
     */
    renderFeatherStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
     ...
     */
    renderRunningStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        // Draw dashed line
     ...
     * Render stem stitch
     */
    renderStemStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
     ...
     * Render split stitch
     */
    renderSplitStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
     ...
     * Render cable stitch
     */
    renderCableStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
     ...
     */
    renderHerringboneStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
        // Draw zigzag pattern
        for (let i = 0; i < points.length - 2; i += 2) {
     ...
     */
    renderFishboneStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
     ...
     */
    renderDetachedChain(points, config, ctx, settings) {
        const size = config.thickness * 2;
        for (const point of points) {
     ...
     * Render fly stitch
     */
    renderFlyStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
     ...
     */
    renderWheatEarStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        for (let i = 0; i < points.length - 1; i++) {
            const p1 = points[i];
     ...
     */
    renderSpiderWebStitch(points, config, ctx, settings) {
        if (points.length < 3)
            return;
        ...
        const maxRadius = Math.max(...points.slice(1).map(p => Math.sqrt((p.x - center.x) ** 2 + (p.y - center.y) ** 2)));
     ...
     * Render brick stitch
     */
    renderBrickStitch(points, config, ctx, settings) {
        const brickWidth = config.thickness * 3;
        const brickHeight = config.thickness * 2;
        for (let i = 0; i < points.length; i++) {
            const point = points[i];
            const offsetX = (i % 2) * brickWidth / 2;
            ctx.fillRect(point.x - brickWidth / 2 + offsetX, point.y - brickHeight / 2, brickWidth, brickHeight);
        }
    }
    /**
     * Render seed stitch
     */
    renderSeedStitch(points, config, ctx, settings) {
     ...
     * Render coral stitch
     */
    renderCoralStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        for (let i = 0; i < points.length - 1; i++) {
            const p1 = points[i];
     ...
     */
    renderTurkishStitch(points, config, ctx, settings) {
        const size = config.thickness * 2;
        for (const point of points) {
            // Draw Turkish knot
            ctx.beginPath();
     ...
     */
    renderRibbonStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        const width = config.thickness * 2;
        for (let i = 0; i < points.length - 1; i++) {
     ...
     */
    renderSimpleStitch(points, config, ctx, settings) {
        if (points.length < 2)
            return;
        ctx.beginPath();
        ctx.moveTo(points[0].x, points[0].y);
     ...
     */
    addTextureLines(points, config, ctx) {
     ...
     */
    generateCacheKey(points, config, options) {
