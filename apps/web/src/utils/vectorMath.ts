...
export function perpendicular(p1: Point2D, p2: Point2D, distance: number = 1): Point2D {
  const dx = p2.x - p1.x;
  const dy = p2.y - p1.y;
...
export function calculateSmoothControlPoints(
  prev: Point2D | null,
  current: Point2D,
  next: Point2D | null,
  tension: number = 0.5
): { controlIn: Point2D; controlOut: Point2D } {
  const controlIn: Point2D = { x: 0, y: 0 };
  const controlOut: Point2D = { x: 0, y: 0 };
...
export function calculateSymmetricControlPoints(
  prev: Point2D | null,
  current: Point2D,
  next: Point2D | null,
  tension: number = 0.5
...
export function snapToPoint(point: Point2D, snapPoints: Point2D[], snapDistance: number = 10): Point2D {
...
export function calculateCatmullRomSpline(points: Point2D[], tension: number = 0.5): Point2D[] {
  if (points.length < 2) return points;
  if (points.length === 2) return points;
...
function calculateCatmullRomSegment(
  p0: Point2D,
  p1: Point2D,
  p2: Point2D,
  p3: Point2D,
