...
export function ProceduralGenerator({ active }) {
    // Console log removed
    const [selectedPattern, setSelectedPattern] = useState('perlin_noise');
    ...
    const generatePerlinNoise = async (ctx, width, height) => {
        // Console log removed
        const imageData = ctx.createImageData(width, height);
        const data = imageData.data;
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                const noise = perlinNoise2D(x * noiseSettings.frequency, y * noiseSettings.frequency, generatorSettings.octaves, generatorSettings.persistence, generatorSettings.lacunarity, generatorSettings.seed);
    ...
    const generateSimplexNoise = async (ctx, width, height) => {
        // Console log removed
        // Simplified Simplex noise implementation
        await generatePerlinNoise(ctx, width, height);
    };
    const generateWorleyNoise = async (ctx, width, height) => {
        // Console log removed
        const imageData = ctx.createImageData(width, height);
        const data = imageData.data;
        const cellSize = 50;
    ...
    const generateGeometricGrid = async (ctx, width, height) => {
        // Console log removed
    ...
    const generateHexagonalPattern = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        const hexSize = 30;
    ...
    const generateTriangularPattern = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
    ...
    const generateCircularPattern = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
    ...
    const generateOrganicFlow = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        ctx.strokeStyle = '#FFFFFF';
    ...
    const generateVeinPattern = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        ctx.strokeStyle = '#FFFFFF';
        ...
        for (let i = 0; i < 20; i++) {
            ...
            ctx.lineTo(trunkX + Math.cos(branchAngle) * branchLength, branchY + Math.sin(branchAngle) * branchLength);
            ctx.stroke();
        }
    };
    const generateCellPattern = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        const cellSize = 60;
    ...
    const generateMandelbrot = async (ctx, width, height) => {
        // Console log removed
        const imageData = ctx.createImageData(width, height);
        const data = imageData.data;
    ...
    const generateJuliaSet = async (ctx, width, height) => {
        // Console log removed
        // Simplified Julia set implementation
        await generateMandelbrot(ctx, width, height);
    };
    const generateDragonCurve = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        ctx.strokeStyle = '#FFFFFF';
    ...
    const generateVoronoi = async (ctx, width, height) => {
        // Console log removed
        const imageData = ctx.createImageData(width, height);
        const data = imageData.data;
        const points = [];
    ...
    const generateDelaunay = async (ctx, width, height) => {
        // Console log removed
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);
        const points = [];
    ...
    // Helper functions
    const perlinNoise2D = (x, y, octaves, persistence, lacunarity, seed) => {
        let value = 0;
        let amplitude = 1;
        let frequency = 1;
    ...
    return (_jsxs("div", { className: "procedural-generator", children: [_jsxs("div", { className: "tool-header", children: [_jsx("h4", { style: { margin: 0, color: '#8B5CF6', fontSize: '18px' }, children: "\uD83C\uDFB2 Procedural Generator" }), _jsxs("div", { className: "tool-controls", children: [_jsxs("button", { onClick: generatePattern, disabled: isGenerating, style: {
                                    padding: '8px 16px',
                                    ...
                                    alignItems: 'center',
                                    gap: '4px'
                                }, children: [isGenerating ? '⏳' : '🎨', " ", isGenerating ? 'Generating...' : 'Generate'] }), _jsx("button", { onClick: () => useApp.getState().setTool('brush'), style: {
                                    padding: '8px 16px',
                                    background: '#EF4444',
                                    color: 'white',
                                    border: 'none',
                                    ...
                                    fontSize: '12px',
                                    cursor: 'pointer'
                                }, title: "Close Procedural Generator", children: "\u2715 Close" })] })] }), _jsxs("div", { className: "pattern-types", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }, children: "Pattern Types" }), _jsx("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))',
                            gap: '6px',
                            maxHeight: '200px',
                            overflowY: 'auto'
                        }, children: patternTypes.map(pattern => (_jsxs("button", { className: `pattern-btn ${selectedPattern === pattern.id ? 'active' : ''}`, onClick: () => setSelectedPattern(pattern.id), style: {
                                padding: '6px',
                            ...
                            }, children: [_jsx("div", { style: { fontSize: '14px', marginBottom: '2px' }, children: pattern.icon }), _jsx("div", { children: pattern.name })] }, pattern.id))) })] }), _jsxs("div", { className: "generator-settings", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }, children: "Generator Settings" }), _jsxs("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(2, 1fr)',
                            gap: '8px'
                        }, children: [_jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Width" }), _jsx("input", { type: "number", value: generatorSettings.width, onChange: (e) => setGeneratorSettings(prev => ({ ...prev, width: parseInt(e.target.value) })), style: {
                                            width: '100%',
                                            padding: '4px',
                                            background: '#374151',
                                            color: '#F9FAFB',
                                            ...
                                            borderRadius: '4px',
                                            fontSize: '11px'
                                        } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Height" }), _jsx("input", { type: "number", value: generatorSettings.height, onChange: (e) => setGeneratorSettings(prev => ({ ...prev, height: parseInt(e.target.value) })), style: {
                                            width: '100%',
                                            padding: '4px',
                                            background: '#374151',
                                            color: '#F9FAFB',
                                            ...
                                            borderRadius: '4px',
                                            fontSize: '11px'
                                        } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Scale" }), _jsx("input", { type: "range", min: "10", max: "200", step: "10", value: generatorSettings.scale, onChange: (e) => setGeneratorSettings(prev => ({ ...prev, scale: parseInt(e.target.value) })), style: { width: '100%' } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Octaves" }), _jsx("input", { type: "range", min: "1", max: "8", step: "1", value: generatorSettings.octaves, onChange: (e) => setGeneratorSettings(prev => ({ ...prev, octaves: parseInt(e.target.value) })), style: { width: '100%' } })] })] })] }), _jsxs("div", { className: "generated-textures", style: { marginBottom: '16px' }, children: [_jsxs("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }, children: ["Generated Textures (", generatedTextures.length, ")"] }), _jsx("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))',
                            gap: '8px',
                            maxHeight: '200px',
                            overflowY: 'auto'
                        }, children: generatedTextures.map(texture => (_jsx("div", { style: {
                                aspectRatio: '1',
                                background: '#374151',
                                borderRadius: '4px',
                                border: '1px solid rgba(139, 92, 246, 0.3)',
                                ...
                                overflow: 'hidden'
                                    ...
                                    padding: '2px',
                                    textAlign: 'center'
                                }, children: texture.name }) }, texture.id))) })] }), _jsxs("div", { className: "preview-canvas", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }, children: "Preview" }), _jsx("canvas", { ref: previewCanvasRef, width: 300, height: 200, style: {
                            width: '100%',
                            height: '200px',
                            border: '1px solid rgba(139, 92, 246, 0.3)',
                            borderRadius: '4px',
                            background: '#1F2937'
                        } })] }), _jsx("canvas", { ref: canvasRef, width: 512, height: 512, style: { display: 'none' } })] }));
}
