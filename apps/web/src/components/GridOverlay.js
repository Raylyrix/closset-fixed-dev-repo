...
export const GridOverlay = ({ canvasRef }) => {
    const overlayRef = useRef(null);
    const containerRef = useRef(null);
    const { showGrid, gridSize, gridColor, gridOpacity, showRulers, rulerUnits, scale, showGuides, guideColor, snapToGrid, snapDistance } = useApp();
    ...
    useEffect(() => {
        window.snapToGridPoint = snapToGridPoint;
        return () => {
            delete window.snapToGridPoint;
        };
    ...
    return (_jsx("div", { ref: containerRef, style: {
            ...
            pointerEvents: 'none',
            zIndex: 0
        }, children: _jsx("canvas", { ref: overlayRef, style: {
                position: 'absolute',
                top: 0,
                left: 0,
                width: '100%',
