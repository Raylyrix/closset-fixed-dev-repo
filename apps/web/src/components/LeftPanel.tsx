  ...
  return (
    <div>
      ...
      <Section title="Images">
        ...
        {useApp(s => s.importedImages).length > 0 && (
          <div style={{ maxHeight: '400px', overflowY: 'auto', marginBottom: 8 }}>
            {useApp(s => s.importedImages).map((img: any) => (
              <div key={img.id} style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 8 }}>
                  <input
                    type="checkbox"
                    checked={img.visible}
                    onChange={(e) => useApp.getState().updateImportedImage(img.id, { visible: e.target.checked })}
                  />
                  <span style={{ fontSize: 11, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: '600' }}>
                    {img.name}
                  </span>
                  <button 
                    className="btn" 
                    onClick={() => useApp.getState().removeImportedImage(img.id)}
                ...
                {/* Position Controls */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 10, marginBottom: 4 }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: 2 }}>X:</label>
                    <input 
                      type="number" 
                      value={img.x} 
                      onChange={(e) => useApp.getState().updateImportedImage(img.id, { x: Number(e.target.value) })}
                      style={{ width: '100%', padding: 4, fontSize: 10 }}
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: 2 }}>Y:</label>
                    <input 
                      type="number" 
                      value={img.y} 
                      onChange={(e) => useApp.getState().updateImportedImage(img.id, { y: Number(e.target.value) })}
                      style={{ width: '100%', padding: 4, fontSize: 10 }}
                    />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 10, marginBottom: 4 }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: 2 }}>Width:</label>
                    <input 
                      type="number" 
                      value={img.width} 
                      onChange={(e) => useApp.getState().updateImportedImage(img.id, { width: Number(e.target.value) })}
                      style={{ width: '100%', padding: 4, fontSize: 10 }}
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: 2 }}>Height:</label>
                    <input 
                      type="number" 
                      value={img.height} 
                      onChange={(e) => useApp.getState().updateImportedImage(img.id, { height: Number(e.target.value) })}
                      style={{ width: '100%', padding: 4, fontSize: 10 }}
                    />
                  </div>
                </div>
                ...
                <div style={{ fontSize: 10 }}>
                  <label style={{ display: 'block', marginBottom: 2 }}>Opacity: {Math.round(img.opacity * 100)}%</label>
                  <input 
                    type="range" 
                    min="0" 
                    max="100" 
                    value={img.opacity * 100} 
                    onChange={(e) => useApp.getState().updateImportedImage(img.id, { opacity: Number(e.target.value) / 100 })}
                    style={{ width: '100%' }}
                  />
                </div>
              </div>
