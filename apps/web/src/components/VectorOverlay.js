...
export function VectorOverlay() {
    ...
    const draggingBoundsRef = useRef(null);
    // subscribe to store
    useEffect(() => {
            ...
            if (selected.includes(shape.id)) {
                ...
                shape.path.points.forEach((p, i) => {
                    ...
                    if (p.controlIn) {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(p.x, p.y);
                        ctx.lineTo(p.x + p.controlIn.x, p.y + p.controlIn.y);
                        ctx.stroke();
                        ctx.setLineDash([]);
                        ctx.beginPath();
                        ctx.arc(p.x + p.controlIn.x, p.y + p.controlIn.y, 3, 0, Math.PI * 2);
                    ...
                    if (p.controlOut) {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(p.x, p.y);
                        ctx.lineTo(p.x + p.controlOut.x, p.y + p.controlOut.y);
                        ctx.stroke();
                        ctx.setLineDash([]);
                        ctx.beginPath();
                        ctx.arc(p.x + p.controlOut.x, p.y + p.controlOut.y, 3, 0, Math.PI * 2);
                        ctx.fillStyle = '#9CA3AF';
                        ctx.fill();
                    }
                });
    ...
    const onMouseDown = (e) => {
        ...
        if (tool === 'pen') {
            const st = vectorStore.getState();
            if (!st.currentPath) {
                const path = { id: `path_${Date.now()}`, points: [{ x: pos.x, y: pos.y, type: 'corner' }], closed: false, fill: true, stroke: true, fillColor: '#ffffff', strokeColor: '#000000', strokeWidth: 2 };
                vectorStore.setState({ currentPath: path });
            }
            else {
                const cp = { ...st.currentPath, points: [...st.currentPath.points, { x: pos.x, y: pos.y, type: 'corner' }] };
    ...
    const onMouseMove = (e) => {
        ...
        const st = vectorStore.getState();
        if (tool === 'pen')
            return; // preview can be added later
    ...
    const onDoubleClick = () => {
        ...
        if (tool === 'pen' && st.currentPath && st.currentPath.points.length >= 3) {
            ...
            const shape = { id: `shape_${Date.now()}`, type: 'path', path, bounds: b };
            vectorStore.setAll({ currentPath: null, shapes: [...st.shapes, shape], selected: [shape.id] });
        }
    };
    return (_jsx("div", { ref: containerRef, style: { position: 'absolute', inset: 0, pointerEvents: 'none' }, children: _jsx("canvas", { ref: canvasRef, width: canvasSize.w, height: canvasSize.h, style: { position: 'absolute', inset: 0, pointerEvents: 'auto' }, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: onMouseUp, onDoubleClick: onDoubleClick }) }));
}
