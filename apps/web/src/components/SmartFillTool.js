...
export function SmartFillTool({ active }) {
    // Console log removed
    const [selectedMode, setSelectedMode] = useState('content_aware');
    ...
    const performContentAwareFill = useCallback(async (sourceCanvas, maskCanvas, targetCanvas) => {
        // Console log removed
    ...
    const performSeamlessFill = useCallback(async (sourceCanvas, maskCanvas, targetCanvas) => {
        // Console log removed
        const sourceCtx = sourceCanvas.getContext('2d');
        const maskCtx = maskCanvas.getContext('2d');
        const targetCtx = targetCanvas.getContext('2d');
    ...
    const performPatternFill = useCallback(async (sourceCanvas, maskCanvas, targetCanvas) => {
        // Console log removed
        const sourceCtx = sourceCanvas.getContext('2d');
        const maskCtx = maskCanvas.getContext('2d');
        const targetCtx = targetCanvas.getContext('2d');
    ...
    const performGradientFill = useCallback(async (sourceCanvas, maskCanvas, targetCanvas) => {
        // Console log removed
        const sourceCtx = sourceCanvas.getContext('2d');
        const maskCtx = maskCanvas.getContext('2d');
        const targetCtx = targetCanvas.getContext('2d');
    ...
    const performAIGeneratedFill = useCallback(async (sourceCanvas, maskCanvas, targetCanvas) => {
        // Console log removed
    ...
    const handleFill = useCallback(async () => {
        ...
        try {
            ...
            if (!sourceCanvas || !maskCanvas || !targetCanvas)
                return;
            // Create mask
            const maskCtx = maskCanvas.getContext('2d');
    ...
    return (_jsxs("div", { className: "smart-fill-tool", children: [_jsxs("div", { className: "tool-header", children: [_jsx("h4", { style: { margin: 0, color: '#EC4899', fontSize: '18px' }, children: "\uD83E\uDD16 Smart Fill Tool" }), _jsxs("div", { className: "tool-controls", children: [_jsx("button", { onClick: clearSelection, style: {
                                    padding: '6px 12px',
                                    background: '#6B7280',
                                    color: 'white',
                                    border: 'none',
                                    ...
                                    fontSize: '12px',
                                    cursor: 'pointer'
                                }, children: "Clear" }), _jsxs("button", { onClick: handleFill, disabled: !selectedArea || isProcessing, style: {
                                    padding: '6px 12px',
                                    ...
                                    alignItems: 'center',
                                    gap: '4px'
                                }, children: [isProcessing ? '⏳' : '🎨', " ", isProcessing ? 'Processing...' : 'Fill'] }), _jsx("button", { onClick: () => useApp.getState().setTool('brush'), style: {
                                    padding: '6px 12px',
                                    background: '#EF4444',
                                    color: 'white',
                                    border: 'none',
                                    ...
                                    fontSize: '12px',
                                    cursor: 'pointer'
                                }, title: "Close Smart Fill", children: "\u2715 Close" })] })] }), _jsxs("div", { className: "fill-modes", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#EC4899', marginBottom: '8px' }, children: "Fill Modes" }), _jsx("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))',
                            gap: '8px'
                        }, children: fillModes.map(mode => (_jsxs("button", { className: `mode-btn ${selectedMode === mode.id ? 'active' : ''}`, onClick: () => setSelectedMode(mode.id), style: {
                                padding: '8px',
                            ...
                            }, children: [_jsx("div", { style: { fontSize: '16px', marginBottom: '4px' }, children: mode.icon }), _jsx("div", { children: mode.name })] }, mode.id))) })] }), _jsxs("div", { className: "fill-settings", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#EC4899', marginBottom: '8px' }, children: "Fill Settings" }), _jsxs("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(2, 1fr)',
                            gap: '8px'
                        }, children: [_jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Tolerance" }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: fillSettings.tolerance, onChange: (e) => setFillSettings(prev => ({ ...prev, tolerance: parseFloat(e.target.value) })), style: { width: '100%' } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Feather" }), _jsx("input", { type: "range", min: "0", max: "10", step: "1", value: fillSettings.feather, onChange: (e) => setFillSettings(prev => ({ ...prev, feather: parseInt(e.target.value) })), style: { width: '100%' } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "Opacity" }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: fillSettings.opacity, onChange: (e) => setFillSettings(prev => ({ ...prev, opacity: parseFloat(e.target.value) })), style: { width: '100%' } })] }), _jsxs("div", { children: [_jsx("label", { style: { fontSize: '10px', color: '#D1D5DB', display: 'block', marginBottom: '4px' }, children: "AI Strength" }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: fillSettings.aiStrength, onChange: (e) => setFillSettings(prev => ({ ...prev, aiStrength: parseFloat(e.target.value) })), style: { width: '100%' } })] })] })] }), _jsxs("div", { className: "advanced-options", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#EC4899', marginBottom: '8px' }, children: "Advanced Options" }), _jsxs("div", { style: {
                            display: 'flex',
                            gap: '12px',
                            flexWrap: 'wrap'
                        }, children: [_jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#D1D5DB' }, children: [_jsx("input", { type: "checkbox", checked: fillSettings.preserveTexture, onChange: (e) => setFillSettings(prev => ({ ...prev, preserveTexture: e.target.checked })) }), "Preserve Texture"] }), _jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#D1D5DB' }, children: [_jsx("input", { type: "checkbox", checked: fillSettings.matchLighting, onChange: (e) => setFillSettings(prev => ({ ...prev, matchLighting: e.target.checked })) }), "Match Lighting"] }), _jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#D1D5DB' }, children: [_jsx("input", { type: "checkbox", checked: fillSettings.matchPerspective, onChange: (e) => setFillSettings(prev => ({ ...prev, matchPerspective: e.target.checked })) }), "Match Perspective"] })] })] }), _jsxs("div", { className: "selection-area", style: { marginBottom: '16px' }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#EC4899', marginBottom: '8px' }, children: "Selection Area" }), _jsx("canvas", { ref: previewCanvasRef, width: 300, height: 200, style: {
                            width: '100%',
                            height: '200px',
                            border: '1px solid rgba(236, 72, 153, 0.3)',
                            borderRadius: '4px',
                            background: '#1F2937',
                            cursor: 'crosshair'
                        }, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp }), selectedArea && (_jsx("div", { style: {
                            position: 'absolute',
                            left: selectedArea.x,
                            top: selectedArea.y,
                            ...
                            pointerEvents: 'none'
                        } }))] }), _jsxs("div", { className: "fill-results", style: { marginBottom: '16px' }, children: [_jsxs("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#EC4899', marginBottom: '8px' }, children: ["Fill Results (", fillResults.length, ")"] }), _jsx("div", { style: {
                            display: 'grid',
                            gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))',
                            gap: '8px',
                            maxHeight: '200px',
                            overflowY: 'auto'
                        }, children: fillResults.map(result => (_jsxs("div", { style: {
                                aspectRatio: '1',
                                background: '#374151',
                                borderRadius: '4px',
                                border: '1px solid rgba(236, 72, 153, 0.3)',
                                ...
                                overflow: 'hidden'
                                        ...
                                        padding: '2px',
                                        textAlign: 'center'
                                    }, children: result.method }), _jsxs("div", { style: {
                                        position: 'absolute',
                                        top: '2px',
                                        right: '2px',
                                        background: 'rgba(0, 0, 0, 0.7)',
                                        ...
                                        padding: '2px',
                                        borderRadius: '2px'
                                    }, children: [Math.round(result.confidence * 100), "%"] })] }, result.id))) })] }), _jsx("canvas", { ref: canvasRef, width: 512, height: 512, style: { display: 'none' } }), _jsx("canvas", { ref: maskCanvasRef, width: 512, height: 512, style: { display: 'none' } })] }));
}
