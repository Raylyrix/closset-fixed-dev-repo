...
export const useShirtRendering = () => {
    ...
    const throttledRender = useCallback((fn, delay = 8) => {
    ...
    const renderVectorsToActiveLayer = useCallback(() => {
        ...
        try {
            ...
            performanceMonitor.trackError('vector_rendering', error);
        }
    }, [composedCanvas, getActiveLayer, composeLayers, brushColor, brushSize, brushOpacity, activeTool]);
    // Render vector path
    const renderVectorPath = useCallback((ctx, path) => {
    ...
    // Render stitch path
    const renderStitchPath = useCallback((ctx, path, stitchType) => {
        if (!path || !path.points || path.points.length === 0)
            return;
    ...
    const renderAnchorPoints = useCallback((ctx, shapes) => {
        if (!vectorMode)
            return;
    ...
    const updateTexture = useCallback((texture) => {
        if (texture) {
