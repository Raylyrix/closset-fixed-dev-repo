...
export function PuffPrintTool({ active }) {
    const { modelScene, composedCanvas, activeTool, brushColor, brushSize, brushOpacity, brushShape, usePressureSize, usePressureOpacity, symmetryY, symmetryZ, commit, puffBrushSize, puffBrushOpacity, puffHeight: globalPuffHeight, puffCurvature: globalPuffCurvature, puffShape: globalPuffShape, puffColor: globalPuffColor, setPuffBrushSize, setPuffBrushOpacity, setPuffHeight, setPuffCurvature, setPuffShape, setPuffColor } = useApp();
    const [puffCanvas, setPuffCanvas] = useState(null);
    const [displacementCanvas, setDisplacementCanvas] = useState(null);
    ...
    const [puffPatterns, setPuffPatterns] = useState([]);
    ...
    const displacementCtxRef = useRef(null);
    const materialCacheRef = useRef(new Map());
    const originalGeometryCache = useRef(new Map());
    const modifiedGeometryCache = useRef(new Map());
    const originalMaterialCache = useRef(new Map());
    const originalModelState = useRef(new Map());
    // Function to safely subdivide geometry for smoother puff effects
    const subdivideGeometry = (geometry, subdivisions) => {
        // Console log removed
        try {
    ...
    const smoothGeometry = (geometry, smoothingIterations) => {
        // Console log removed
        try {
    ...
    const getModifiedGeometry = (mesh, meshId) => {
    ...
    const createPuffTexture = (x, y, size, opacity) => {
        if (!ctxRef.current) {
            // Console log removed
            return;
        }
    ...
    const createDisplacementMap = (x, y, size, opacity) => {
    ...
    const handlePaint = (uv, pressure = 1.0) => {
    ...
    const drawLine = (start, end, pressure = 1.0) => {
        if (!puffCanvas || !displacementCanvas)
            return;
        const size = usePressureSize ? puffBrushSize * pressure : puffBrushSize;
        const opacity = usePressureOpacity ? puffBrushOpacity * pressure : puffBrushOpacity;
    ...
    const handleErase = (uv, pressure = 1.0) => {
        if (!puffCanvas || !displacementCanvas) {
            // Console log removed
            return;
        }
    ...
    useEffect(() => {
        ...
        document.addEventListener('puffPaint', handlePuffPaint);
        document.addEventListener('puffErase', handlePuffErase);
        return () => {
            document.removeEventListener('puffPaint', handlePuffPaint);
            document.removeEventListener('puffErase', handlePuffErase);
        };
    ...
    const updateLayer = (layerId, updates) => {
        setPuffLayers(prev => prev.map(layer => layer.id === layerId ? { ...layer, ...updates } : layer));
    };
    // Pattern painting function
    const paintPattern = (uv, pressure = 1.0) => {
        if (!puffCanvas || !selectedPattern)
            return;
    ...
    if (!active && activeTool !== 'eraser')
        return null;
    return (_jsxs("div", { className: "puff-print-tool", style: {
            ...
            position: 'relative'
                    ...
                    color: 'white',
                    fontWeight: 'bold'
                }, children: "\u2713" }), _jsxs("div", { className: "tool-header", children: [_jsxs("h4", { children: ["\uD83C\uDFA8 Puff Print Tool ", activeTool === 'eraser' && '(Eraser Mode)'] }), _jsxs("div", { className: "tool-controls", children: [_jsx("button", { className: "btn", onClick: clearPuffCanvas, title: "Clear Puff Map", children: "\uD83D\uDDD1" }), _jsx("button", { className: "btn", onClick: exportPuffMap, title: "Export Puff Map", children: "\uD83D\uDCBE" }), _jsx("button", { className: "btn", onClick: () => useApp.getState().setTool('brush'), title: "Close Puff Print Tool", style: { background: '#EF4444', color: 'white' }, children: "\u2715 Close" })] })] }), _jsxs("div", { className: "puff-controls", style: { marginTop: '12px' }, children: [_jsxs("div", { className: "control-group", children: [_jsx("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "Puff Color:" }), _jsx("input", { type: "color", value: puffColor, onChange: (e) => setPuffColor(e.target.value), style: { width: '100%', height: '30px', border: '1px solid #ff69b4', borderRadius: '4px' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Brush Size: ", puffBrushSize] }), _jsx("input", { type: "range", min: "1", max: "100", step: "1", value: puffBrushSize, onChange: (e) => setPuffBrushSize(parseInt(e.target.value)), style: { width: '100%' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Brush Opacity: ", Math.round(puffBrushOpacity * 100), "%"] }), _jsx("input", { type: "range", min: "0.1", max: "1.0", step: "0.1", value: puffBrushOpacity, onChange: (e) => setPuffBrushOpacity(parseFloat(e.target.value)), style: { width: '100%' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Height: ", puffHeight] }), _jsx("input", { type: "range", min: "0.1", max: "5.0", step: "0.1", value: puffHeight, onChange: (e) => setPuffHeight(parseFloat(e.target.value)), style: { width: '100%' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Curvature: ", puffCurvature] }), _jsx("input", { type: "range", min: "0.1", max: "1.0", step: "0.1", value: puffCurvature, onChange: (e) => setPuffCurvature(parseFloat(e.target.value)), style: { width: '100%' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "Shape:" }), _jsxs("select", { value: puffShape, onChange: (e) => setPuffShape(e.target.value), style: { width: '100%', padding: '4px', border: '1px solid #ff69b4', borderRadius: '4px' }, children: [_jsx("option", { value: "sphere", children: "\uD83C\uDF10 Sphere" }), _jsx("option", { value: "cube", children: "\u2B1C Cube" }), _jsx("option", { value: "cylinder", children: "\uD83D\uDD32 Cylinder" }), _jsx("option", { value: "pipe", children: "\u2B55 Pipe" })] })] }), _jsxs("div", { style: { marginTop: '16px', padding: '8px', background: 'rgba(255, 105, 180, 0.05)', borderRadius: '4px', border: '1px solid rgba(255, 105, 180, 0.2)' }, children: [_jsx("h5", { style: { margin: '0 0 8px 0', fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "\uD83C\uDFA8 Displacement Quality" }), _jsxs("div", { style: { fontSize: '10px', color: '#67e8f9', marginTop: '4px', padding: '6px', background: 'rgba(103, 232, 249, 0.1)', borderRadius: '4px' }, children: ["\u2705 ", _jsx("strong", { children: "Safe Mode:" }), " Using displacement maps only - no mesh modification to prevent holes and broken vertices"] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Displacement Quality: ", subdivisionLevel] }), _jsx("input", { type: "range", min: "1", max: "4", step: "1", value: subdivisionLevel, onChange: (e) => setSubdivisionLevel(parseInt(e.target.value)), style: { width: '100%' } }), _jsxs("div", { style: { fontSize: '10px', color: '#ff69b4', marginTop: '2px' }, children: [subdivisionLevel === 1 && 'Basic - Standard displacement quality', subdivisionLevel === 2 && 'Enhanced - Better displacement detail', subdivisionLevel === 3 && 'High - High-quality displacement', subdivisionLevel === 4 && 'Ultra - Maximum displacement quality'] })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4', display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("input", { type: "checkbox", checked: autoSmooth, onChange: (e) => setAutoSmooth(e.target.checked), style: { transform: 'scale(1.2)' } }), "Displacement Smoothing"] }), _jsx("div", { style: { fontSize: '10px', color: '#ff69b4', marginTop: '2px' }, children: autoSmooth ? '✅ Smooth displacement gradients enabled' : '❌ Sharp displacement edges' })] }), autoSmooth && (_jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Smoothing Level: ", smoothingLevel] }), _jsx("input", { type: "range", min: "1", max: "5", step: "1", value: smoothingLevel, onChange: (e) => setSmoothingLevel(parseInt(e.target.value)), style: { width: '100%' } }), _jsxs("div", { style: { fontSize: '10px', color: '#ff69b4', marginTop: '2px' }, children: [smoothingLevel === 1 && 'Light (Sharp edges)', smoothingLevel === 2 && 'Medium (Smooth edges)', smoothingLevel === 3 && 'High (Very smooth)', smoothingLevel === 4 && 'Very High (Ultra smooth)', smoothingLevel === 5 && 'Maximum (Perfect smooth)'] })] }))] })] }), _jsxs("div", { style: { marginTop: '16px', padding: '8px', background: 'rgba(255, 105, 180, 0.05)', borderRadius: '4px', border: '1px solid rgba(255, 105, 180, 0.2)' }, children: [_jsx("h5", { style: { margin: '0 0 8px 0', fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "\uD83C\uDFA8 Advanced Features" }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "Pattern Library:" }), _jsx("div", { style: { display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }, children: puffPatterns.map(pattern => (_jsxs("button", { onClick: () => {
                                        ...
                                        cursor: 'pointer'
                                    }, title: pattern.name, children: [pattern.preview, " ", pattern.name] }, pattern.id))) })] }), isPatternMode && selectedPattern && (_jsxs("div", { style: { marginTop: '8px', padding: '8px', background: 'rgba(255, 105, 180, 0.1)', borderRadius: '4px' }, children: [_jsxs("div", { className: "control-group", children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Pattern Scale: ", patternScale.toFixed(1), "x"] }), _jsx("input", { type: "range", min: "0.5", max: "3.0", step: "0.1", value: patternScale, onChange: (e) => setPatternScale(parseFloat(e.target.value)), style: { width: '100%' } })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: ["Pattern Rotation: ", patternRotation, "\u00B0"] }), _jsx("input", { type: "range", min: "0", max: "360", step: "15", value: patternRotation, onChange: (e) => setPatternRotation(parseInt(e.target.value)), style: { width: '100%' } })] }), _jsx("button", { onClick: () => setIsPatternMode(false), style: {
                                    marginTop: '8px',
                                    padding: '4px 8px',
                                    fontSize: '10px',
                                    background: '#EF4444',
                                    ...
                                    borderRadius: '4px',
                                    cursor: 'pointer'
                                }, children: "Exit Pattern Mode" })] })), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4' }, children: "Layers:" }), _jsx("div", { style: { display: 'flex', gap: '4px', marginTop: '4px' }, children: _jsx("button", { onClick: addLayer, style: {
                                        padding: '4px 8px',
                                        fontSize: '10px',
                                        background: '#10B981',
                                        color: 'white',
                                        ...
                                        borderRadius: '4px',
                                        cursor: 'pointer'
                                    }, children: "+ Add Layer" }) }), _jsx("div", { style: { marginTop: '8px', maxHeight: '120px', overflowY: 'auto' }, children: puffLayers.map(layer => (_jsxs("div", { style: {
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '8px',
                                        padding: '4px',
                                        ...
                                        borderRadius: '4px',
                                        marginBottom: '2px'
                                    }, children: [_jsx("input", { type: "checkbox", checked: layer.visible, onChange: (e) => updateLayer(layer.id, { visible: e.target.checked }), style: { transform: 'scale(0.8)' } }), _jsx("span", { style: { fontSize: '10px', flex: 1 }, children: layer.name }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: layer.opacity, onChange: (e) => updateLayer(layer.id, { opacity: parseFloat(e.target.value) }), style: { width: '60px' } }), _jsx("button", { onClick: () => setActiveLayer(layer.id), style: {
                                                padding: '2px 6px',
                                                fontSize: '8px',
                                                ...
                                                borderRadius: '2px',
                                                cursor: 'pointer'
                                            }, children: activeLayer === layer.id ? 'Active' : 'Select' }), puffLayers.length > 1 && (_jsx("button", { onClick: () => deleteLayer(layer.id), style: {
                                                padding: '2px 6px',
                                                fontSize: '8px',
                                                background: '#EF4444',
                                                color: 'white',
                                                ...
                                                borderRadius: '2px',
                                                cursor: 'pointer'
                                            }, children: "\u00D7" }))] }, layer.id))) })] }), _jsxs("div", { className: "control-group", style: { marginTop: '8px' }, children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#ff69b4', display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("input", { type: "checkbox", checked: showGrid, onChange: (e) => setShowGrid(e.target.checked), style: { transform: 'scale(1.2)' } }), "Show Grid"] }), showGrid && (_jsxs("div", { style: { marginTop: '4px' }, children: [_jsxs("label", { style: { fontSize: '10px', color: '#ff69b4' }, children: ["Grid Size: ", gridSize, "px"] }), _jsx("input", { type: "range", min: "10", max: "50", step: "5", value: gridSize, onChange: (e) => setGridSize(parseInt(e.target.value)), style: { width: '100%' } }), _jsxs("label", { style: { fontSize: '10px', color: '#ff69b4', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }, children: [_jsx("input", { type: "checkbox", checked: snapToGrid, onChange: (e) => setSnapToGrid(e.target.checked), style: { transform: 'scale(0.8)' } }), "Snap to Grid"] })] }))] })] }), _jsxs("div", { className: "tool-info", style: { marginTop: '12px' }, children: [_jsxs("p", { children: ["\u2705 ", _jsx("strong", { children: "Active!" }), " Paint on the model to create realistic puff print effects."] }), _jsx("p", { children: "Use the controls above to adjust puff properties." }), _jsxs("p", { children: ["Current brush: ", brushShape, " | Size: ", puffBrushSize, " | Opacity: ", Math.round(puffBrushOpacity * 100), "%"] }), activeTool === 'eraser' && (_jsxs("p", { style: { color: '#ff6b6b', fontSize: '12px', fontWeight: 'bold' }, children: ["\uD83E\uDDFD ", _jsx("strong", { children: "Eraser Mode:" }), " Erasing puff print effects - click and drag to remove puff effects"] })), isPatternMode && selectedPattern && (_jsxs("p", { style: { color: '#10B981', fontSize: '12px', fontWeight: 'bold' }, children: ["\uD83C\uDFA8 ", _jsx("strong", { children: "Pattern Mode:" }), " ", puffPatterns.find(p => p.id === selectedPattern)?.name, " - Click to place patterns"] })), _jsxs("p", { style: { color: '#67e8f9', fontSize: '12px' }, children: ["\uD83C\uDFA8 ", _jsx("strong", { children: "Displacement Quality:" }), " Controls the smoothness of puff effects without modifying mesh geometry"] }), _jsxs("p", { style: { color: '#67e8f9', fontSize: '12px' }, children: ["\uD83D\uDD27 ", _jsx("strong", { children: "Displacement Smoothing:" }), " Creates smooth gradients for professional puff effects"] }), _jsxs("p", { style: { color: '#4ade80', fontSize: '12px' }, children: ["\u2705 ", _jsx("strong", { children: "Safe Mode:" }), " No mesh modification - prevents holes and broken vertices"] }), _jsxs("p", { style: { color: '#F59E0B', fontSize: '12px' }, children: ["\uD83C\uDFAD ", _jsx("strong", { children: "Pattern Library:" }), " Choose from predefined patterns for consistent designs"] }), _jsxs("p", { style: { color: '#F59E0B', fontSize: '12px' }, children: ["\uD83D\uDCDA ", _jsx("strong", { children: "Layer System:" }), " Create complex designs with multiple puff layers"] }), _jsxs("p", { style: { color: '#F59E0B', fontSize: '12px' }, children: ["\uD83D\uDCD0 ", _jsx("strong", { children: "Grid Tools:" }), " Enable grid for precise alignment and measurements"] }), puffCanvas && (_jsx("p", { style: { color: '#67e8f9', fontSize: '12px' }, children: "\uD83D\uDCA1 Tip: Click and drag on the model to create raised puff effects" }))] })] }));
}
