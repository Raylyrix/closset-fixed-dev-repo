...
export function AdvancedSelection({ active }) {
    // Console log removed
    const { composedCanvas, activeTool, brushColor, brushSize, layers, activeLayerId, commit } = useApp();
    // Selection state
    const [selectionType, setSelectionType] = useState('rectangular');
    ...
    const performMagicWandSelection = useCallback((x, y) => {
        ...
        while (stack.length > 0) {
            ...
            if (current.x < 0 || current.x >= composedCanvas.width ||
                current.y < 0 || current.y >= composedCanvas.height)
                continue;
            visited.add(key);
            ...
            const colorDistance = Math.sqrt(Math.pow(pixelColor.r - targetColor.r, 2) +
    ...
    // Lasso selection
    const performLassoSelection = useCallback((points) => {
    ...
    const performRectangularSelection = useCallback((area) => {
        if (!composedCanvas || !selectionCanvasRef.current) {
            // Console log removed
            return;
        }
    ...
    const performEllipticalSelection = useCallback((area) => {
        if (!composedCanvas || !selectionCanvasRef.current) {
            // Console log removed
            return;
        }
    ...
    const performQuickSelection = useCallback((x, y) => {
        if (!composedCanvas || !selectionCanvasRef.current) {
            // Console log removed
            return;
        }
    ...
    const handleMouseMove = useCallback((e) => {
        if (!isSelecting || !isDrawingRef.current)
            return;
        const rect = e.target.getBoundingClientRect();
        const x = e.clientX - rect.left;
        const y = e.clientY - rect.top;
    ...
    useEffect(() => {
        if (!previewCanvasRef.current || !composedCanvas)
            return;
        // Console log removed
    ...
    // Console log removed
    return (_jsxs("div", { className: "advanced-selection", style: {
            ...
            marginTop: '12px'
        }, children: [_jsxs("div", { className: "selection-header", style: {
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
                    marginBottom: '12px'
                }, children: [_jsx("h4", { style: { margin: 0, color: '#3B82F6', fontSize: '16px' }, children: "\uD83C\uDFAF Advanced Selection" }), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("button", { className: "btn", onClick: applySelectionToLayer, disabled: !selectionMask, style: {
                                    background: selectionMask ? '#10B981' : '#6B7280',
                                    color: 'white',
                                    fontSize: '12px',
                                    padding: '6px 12px'
                                }, children: "Apply" }), _jsx("button", { className: "btn", onClick: clearSelection, style: {
                                    background: '#EF4444',
                                    color: 'white',
                                    fontSize: '12px',
                                    padding: '6px 12px'
                                }, children: "Clear" }), _jsx("button", { className: "btn", onClick: () => useApp.getState().setTool('brush'), style: {
                                    background: '#6B7280',
                                    color: 'white',
                                    fontSize: '12px',
                                    padding: '6px 12px'
                                }, title: "Close Advanced Selection", children: "\u2715 Close" })] })] }), _jsx("div", { className: "selection-tools", style: {
                    display: 'grid',
                    ...
                    { id: 'quickSelection', label: 'Quick', icon: '⚡' }
                ].map(tool => (_jsxs("button", { className: `btn ${selectionType === tool.id ? 'active' : ''}`, onClick: () => {
                        // Console log removed
                        setSelectionType(tool.id);
                        clearSelection();
                    }, style: {
                        fontSize: '10px',
                        padding: '8px 4px',
                        ...
                        alignItems: 'center',
                        gap: '4px'
                    }, children: [_jsx("span", { style: { fontSize: '16px' }, children: tool.icon }), _jsx("span", { children: tool.label })] }, tool.id))) }), _jsxs("div", { className: "selection-settings", style: {
                    display: 'grid',
                    gridTemplateColumns: '1fr 1fr',
                    gap: '12px',
                    marginBottom: '12px'
                }, children: [_jsxs("div", { children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#3B82F6' }, children: ["Tolerance: ", tolerance] }), _jsx("input", { type: "range", min: "1", max: "100", value: tolerance, onChange: (e) => {
                                    // Console log removed
                                    setTolerance(parseInt(e.target.value));
                                }, style: { width: '100%' } })] }), _jsxs("div", { children: [_jsxs("label", { style: { fontSize: '12px', fontWeight: 'bold', color: '#3B82F6' }, children: ["Feather: ", feather, "px"] }), _jsx("input", { type: "range", min: "0", max: "20", value: feather, onChange: (e) => {
                                    // Console log removed
                                    setFeather(parseInt(e.target.value));
                                }, style: { width: '100%' } })] }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("input", { type: "checkbox", id: "antiAliasing", checked: antiAliasing, onChange: (e) => {
                                    // Console log removed
                                    setAntiAliasing(e.target.checked);
                                } }), _jsx("label", { htmlFor: "antiAliasing", style: { fontSize: '12px', color: '#3B82F6' }, children: "Anti-aliasing" })] }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("input", { type: "checkbox", id: "contiguous", checked: contiguous, onChange: (e) => {
                                    // Console log removed
                                    setContiguous(e.target.checked);
                                } }), _jsx("label", { htmlFor: "contiguous", style: { fontSize: '12px', color: '#3B82F6' }, children: "Contiguous" })] })] }), selectionMask && (_jsxs("div", { className: "selection-preview", style: {
                    border: '1px solid #3B82F6',
                    borderRadius: '4px',
                    padding: '8px',
                    background: 'white',
                    marginBottom: '12px'
                }, children: [_jsx("div", { style: { fontSize: '12px', fontWeight: 'bold', color: '#3B82F6', marginBottom: '8px' }, children: "Selection Preview" }), _jsx("canvas", { ref: previewCanvasRef, width: composedCanvas?.width || 400, height: composedCanvas?.height || 400, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, style: {
                            width: '100%',
                            height: 'auto',
                            maxWidth: '300px',
                            cursor: 'crosshair',
                            border: '1px solid #E5E7EB'
                        } })] })), _jsxs("div", { style: { fontSize: '12px', color: '#6B7280', textAlign: 'center' }, children: [selectionType === 'magicWand' && 'Click on an area to select similar colors', selectionType === 'quickSelection' && 'Click and drag to select objects', selectionType === 'rectangular' && 'Click and drag to create rectangular selection', selectionType === 'elliptical' && 'Click and drag to create elliptical selection', selectionType === 'lasso' && 'Click and drag to draw freehand selection', selectionType === 'polygonalLasso' && 'Click to add points, double-click to finish', selectionType === 'magneticLasso' && 'Click and drag along edges for magnetic selection'] })] }));
}
