...
export function ColorGrading({ active }: ColorGradingProps) {
  // Console log removed
  ...
  // Color grading state
  const [adjustments, setAdjustments] = useState<ColorAdjustment>({
    brightness: 0,
    contrast: 0,
    saturation: 0,
    vibrance: 0,
  ...
  const [colorCurves, setColorCurves] = useState<ColorCurve[]>([]);
  ...
  // Color lookup tables
  const [colorLookups, setColorLookups] = useState<ColorLookup[]>([
    {
      id: 'vintage',
      name: 'Vintage',
  ...
  // Refs
  const gradingCanvasRef = useRef<HTMLCanvasElement>(null);
  ...
  const generateHistogram = useCallback((canvas: HTMLCanvasElement) => {
    // Console log removed
  ...
  const applyColorAdjustments = useCallback((imageData: ImageData, adjustments: ColorAdjustment): ImageData => {
    // Console log removed
  ...
  // Apply color lookup
  const applyColorLookup = useCallback((lookup: ColorLookup) => {
    // Console log removed
  ...
  return (
    <div className="color-grading" style={{
      ...
      <div className="grading-header" style={{
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button
            className="btn"
            onClick={applyColorGradingToDesign}
            style={{
              background: '#10B981',
              color: 'white',
              fontSize: '12px',
      ...
      {/* Color Lookups */}
      <div className="color-lookups" style={{
        marginBottom: '12px'
      }}>
        ...
        <div style={{
          ...
          {colorLookups.map(lookup => (
            <button
              key={lookup.id}
              className="btn"
              onClick={() => applyColorLookup(lookup)}
              style={{
                fontSize: '10px',
                padding: '8px 4px',
                display: 'flex',
      ...
      {/* Basic Adjustments */}
      <div className="basic-adjustments" style={{
        marginBottom: '12px'
      }}>
        <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#F59E0B', marginBottom: '8px' }}>
          Basic Adjustments
        ...
        <div style={{
          ...
          <div>
            <label style={{ fontSize: '10px', color: '#F59E0B' }}>
      ...
      {/* Color Adjustments */}
      <div className="color-adjustments" style={{
        marginBottom: '12px'
      }}>
        <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#F59E0B', marginBottom: '8px' }}>
          Color Adjustments
      ...
      {/* Tone Adjustments */}
      <div className="tone-adjustments" style={{
        marginBottom: '12px'
      }}>
        <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#F59E0B', marginBottom: '8px' }}>
          Tone Adjustments
        ...
        <div style={{
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setAdjustments(prev => ({ ...prev, whites: parseInt(e.target.value) }))}
              style={{ width: '100%' }}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setAdjustments(prev => ({ ...prev, blacks: parseInt(e.target.value) }))}
              style={{ width: '100%' }}
            />
          </div>
        </div>
      ...
      {isPreviewMode && previewCanvas && (
        <div className="grading-preview" style={{
