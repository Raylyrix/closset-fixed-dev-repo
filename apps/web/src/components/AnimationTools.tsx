...
export function AnimationTools({ active }: AnimationToolsProps) {
  // Console log removed
  ...
  const createAnimation = useCallback((name: string, fps: number = 30) => {
    // Console log removed
  ...
  const addFrame = useCallback((animation: Animation, canvas: HTMLCanvasElement, duration: number = 100) => {
    // Console log removed
  ...
  const applyAnimationPreset = useCallback((preset: AnimationPreset) => {
  ...
  const interpolateProperties = useCallback((props1: any, props2: any, t: number) => {
    const result: any = {};
  ...
  // Export animation
  const exportAnimation = useCallback((animation: Animation, format: 'gif' | 'mp4' | 'webm' = 'gif') => {
    // Console log removed
  ...
  const renderTimeline = useCallback(() => {
    if (!timelineCanvasRef.current || !currentAnimation) return;
  ...
  return (
    <div className="animation-tools" style={{
      ...
      <div className="animation-header" style={{
        display: 'flex',
      ...
      {/* Animation Presets */}
      <div className="animation-presets" style={{
        marginBottom: '12px'
      }}>
        <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }}>
          Animation Presets
      ...
      <div className="playback-controls" style={{
        ...
        <div style={{
          ...
          <div>
            <label style={{ fontSize: '10px', color: '#8B5CF6' }}>
          ...
          <div>
            <label style={{ fontSize: '10px', color: '#8B5CF6' }}>
              Frame: {currentFrame} / {currentAnimation?.frames.length || 0}
            </label>
            <input
              type="range"
              min="0"
              max={currentAnimation?.frames.length ? currentAnimation.frames.length - 1 : 0}
              value={currentFrame}
      ...
      {currentAnimation && (
        <div className="timeline" style={{
          ...
          <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#8B5CF6', marginBottom: '8px' }}>
            Timeline - {currentAnimation.name}
          </div>
          <canvas
      ...
      {/* Animation Preview */}
      {currentAnimation && currentAnimation.frames.length > 0 && (
      ...
      <div className="export-options" style={{
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            className="btn"
            onClick={() => currentAnimation && exportAnimation(currentAnimation, 'gif')}
            disabled={!currentAnimation || currentAnimation.frames.length === 0}
            style={{
              background: '#10B981',
              color: 'white',
          ...
          <button
            className="btn"
            onClick={() => currentAnimation && exportAnimation(currentAnimation, 'mp4')}
            disabled={!currentAnimation || currentAnimation.frames.length === 0}
            style={{
              background: '#8B5CF6',
              color: 'white',
      ...
      <div style={{ fontSize: '12px', color: '#6B7280', textAlign: 'center' }}>
        Create and animate your designs with professional motion graphics tools
      </div>
    </div>
  );
}
