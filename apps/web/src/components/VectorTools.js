...
export function VectorTools({ active }) {
    // Console log removed
    ...
    const [defaultFillType, setDefaultFillType] = useState('solid');
    const [defaultGradient, setDefaultGradient] = useState({ type: 'linear', color1: '#FF6B6B', color2: '#4F46E5', angle: 0 });
    const [defaultStrokeJoin, setDefaultStrokeJoin] = useState('round');
    const [defaultStrokeCap, setDefaultStrokeCap] = useState('round');
    const [defaultStrokeColor, setDefaultStrokeColor] = useState(brushColor);
    const [defaultStrokeWidth, setDefaultStrokeWidth] = useState(brushSize);
    const [defaultStrokeEnabled, setDefaultStrokeEnabled] = useState(true);
    const [defaultFillColor, setDefaultFillColor] = useState(brushColor);
    const [defaultFillOpacity, setDefaultFillOpacity] = useState(1.0);
    const [defaultStrokeOpacity, setDefaultStrokeOpacity] = useState(1.0);
    const [imageFillState, setImageFillState] = useState({ src: null, img: undefined, scale: 1, offsetX: 0, offsetY: 0 });
    ...
    const [vectorCanvas, setVectorCanvas] = useState(null);
    ...
    // Create new path
    const createNewPath = useCallback((startPoint) => {
        const newPath = {
            id: `path_${Date.now()}`,
            points: [startPoint],
            closed: false,
    ...
    const calculatePathBounds = useCallback((path) => {
    ...
    // Draw Bezier curve
    const drawBezierCurve = useCallback((ctx, points) => {
        if (points.length < 2)
            return;
        // Console log removed
        ctx.beginPath();
    ...
    const renderVectorShapes = useCallback(() => {
        ...
        if (currentPath && currentPath.points.length > 0) {
            ...
            previewCtx.save();
            if (currentPath.fill) {
                const tempShape = { id: 'temp', type: 'path', path: currentPath, bounds: calculatePathBounds(currentPath) };
    ...
    const performPathOperation = useCallback((operation) => {
    ...
    const hitTestHandle = (pt, shape) => {
        for (let i = 0; i < shape.path.points.length; i++) {
            ...
            if (hout) {
                const dx = pt.x - hout.x;
                const dy = pt.y - hout.y;
                if (dx * dx + dy * dy < 7 * 7)
    ...
    return (_jsxs("div", { style: {
            ...
            overflowY: 'auto'
                    ...
                    marginBottom: '16px',
                    textAlign: 'center'
                }, children: _jsx("h3", { style: { margin: 0, fontSize: '18px', fontWeight: 600 }, children: "\uD83C\uDFA8 Vector Tools" }) }), _jsxs("div", { style: { marginBottom: '16px' }, children: [_jsx("div", { style: { marginBottom: '8px', fontWeight: 500, color: '#E2E8F0' }, children: "Tool" }), _jsx("div", { style: { display: 'flex', gap: '6px', flexWrap: 'wrap' }, children: ['pen', 'pathSelection', 'addAnchor', 'removeAnchor', 'convertAnchor', 'curvature'].map(tool => (_jsx("button", { className: `btn ${vectorTool === tool ? 'active' : ''}`, onClick: () => {
                                ...
                                fontSize: '12px',
                                cursor: 'pointer'
                            }, children: tool.charAt(0).toUpperCase() + tool.slice(1).replace(/([A-Z])/g, ' $1') }, tool))) })] }), _jsxs("div", { style: { marginBottom: '16px' }, children: [_jsx("div", { style: { marginBottom: '12px', fontWeight: 500, color: '#E2E8F0' }, children: "Fill & Stroke" }), _jsxs("div", { style: {
                            background: 'rgba(139, 92, 246, 0.1)',
                            padding: '12px',
                            borderRadius: '8px',
                            marginBottom: '12px',
                            border: '1px solid rgba(139, 92, 246, 0.3)'
                        }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', marginBottom: '8px' }, children: [_jsx("input", { type: "checkbox", checked: defaultFillType !== 'none', onChange: (e) => {
                                            setDefaultFillType(e.target.checked ? 'solid' : 'none');
                                            applySettingsToSelected();
                                        }, style: { marginRight: '8px' } }), _jsx("span", { style: { fontWeight: 500 }, children: "Fill" })] }), defaultFillType !== 'none' && (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Fill Type" }), _jsxs("select", { value: defaultFillType, onChange: (e) => {
                                                    setDefaultFillType(e.target.value);
                                                    applySettingsToSelected();
                                                }, style: {
                                                    width: '100%',
                                                    padding: '6px',
                                                    ...
                                                    color: 'white'
                                                }, children: [_jsx("option", { value: "solid", children: "Solid Color" }), _jsx("option", { value: "linear", children: "Linear Gradient" }), _jsx("option", { value: "radial", children: "Radial Gradient" }), _jsx("option", { value: "image", children: "Image Fill" })] })] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Fill Color" }), _jsx("input", { type: "color", value: defaultFillType === 'solid' ? defaultFillColor : defaultGradient.color1, onChange: (e) => {
                                                    ...
                                                    borderRadius: '4px',
                                                    cursor: 'pointer'
                                                } })] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsxs("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: ["Fill Opacity: ", Math.round(defaultFillOpacity * 100), "%"] }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: defaultFillOpacity, onChange: (e) => {
                                                    setDefaultFillOpacity(Number(e.target.value));
                                                    applySettingsToSelected();
                                                }, style: { width: '100%' } })] }), (defaultFillType === 'linear' || defaultFillType === 'radial') && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Gradient Color 2" }), _jsx("input", { type: "color", value: defaultGradient.color2, onChange: (e) => setDefaultGradient(prev => ({ ...prev, color2: e.target.value })), style: {
                                                    width: '100%',
                                                    height: '32px',
                                                    border: '1px solid rgba(139, 92, 246, 0.3)',
                                                    borderRadius: '4px',
                                                    cursor: 'pointer'
                                                } })] })), defaultFillType === 'linear' && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsxs("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: ["Angle: ", defaultGradient.angle, "\u00B0"] }), _jsx("input", { type: "range", min: "0", max: "360", value: defaultGradient.angle, onChange: (e) => setDefaultGradient(prev => ({ ...prev, angle: Number(e.target.value) })), style: { width: '100%' } })] }))] }))] }), _jsxs("div", { style: {
                            background: 'rgba(139, 92, 246, 0.1)',
                            padding: '12px',
                            borderRadius: '8px',
                            border: '1px solid rgba(139, 92, 246, 0.3)'
                        }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', marginBottom: '8px' }, children: [_jsx("input", { type: "checkbox", checked: defaultStrokeEnabled, onChange: (e) => {
                                            setDefaultStrokeEnabled(e.target.checked);
                                            applySettingsToSelected();
                                        }, style: { marginRight: '8px' } }), _jsx("span", { style: { fontWeight: 500 }, children: "Stroke" })] }), defaultStrokeEnabled && (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Stroke Color" }), _jsx("input", { type: "color", value: defaultStrokeColor, onChange: (e) => {
                                                    ...
                                                    borderRadius: '4px',
                                                    cursor: 'pointer'
                                                } })] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsxs("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: ["Stroke Width: ", defaultStrokeWidth, "px"] }), _jsx("input", { type: "range", min: "1", max: "20", value: defaultStrokeWidth, onChange: (e) => {
                                                    setDefaultStrokeWidth(Number(e.target.value));
                                                    applySettingsToSelected();
                                                }, style: { width: '100%' } })] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsxs("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: ["Stroke Opacity: ", Math.round(defaultStrokeOpacity * 100), "%"] }), _jsx("input", { type: "range", min: "0", max: "1", step: "0.1", value: defaultStrokeOpacity, onChange: (e) => {
                                                    setDefaultStrokeOpacity(Number(e.target.value));
                                                    applySettingsToSelected();
                                                }, style: { width: '100%' } })] })] })), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Line Join" }), _jsxs("select", { value: defaultStrokeJoin, onChange: (e) => {
                                            setDefaultStrokeJoin(e.target.value);
                                            applySettingsToSelected();
                                        }, style: {
                                            width: '100%',
                                            padding: '6px',
                                            ...
                                            background: 'rgba(15, 23, 42, 0.8)',
                                            color: 'white'
                                        }, children: [_jsx("option", { value: "round", children: "Round" }), _jsx("option", { value: "bevel", children: "Bevel" }), _jsx("option", { value: "miter", children: "Miter" })] })] }), _jsxs("div", { children: [_jsx("div", { style: { marginBottom: '4px', fontSize: '12px', color: '#CBD5E1' }, children: "Line Cap" }), _jsxs("select", { value: defaultStrokeCap, onChange: (e) => {
                                            setDefaultStrokeCap(e.target.value);
                                            applySettingsToSelected();
                                        }, style: {
                                            width: '100%',
                                            padding: '6px',
                                            ...
                                            background: 'rgba(15, 23, 42, 0.8)',
                                            color: 'white'
                                        }, children: [_jsx("option", { value: "round", children: "Round" }), _jsx("option", { value: "butt", children: "Butt" }), _jsx("option", { value: "square", children: "Square" })] })] })] })] }), _jsxs("div", { style: { display: 'flex', gap: '8px', flexDirection: 'column' }, children: [selectedShapes.length > 0 && (_jsxs("button", { onClick: applySettingsToSelected, style: {
                            width: '100%',
                            padding: '8px 16px',
                            borderRadius: '6px',
                            ...
                            cursor: 'pointer',
                            fontSize: '14px'
                        }, children: ["Apply to Selected (", selectedShapes.length, ")"] })), _jsx("button", { onClick: () => {
