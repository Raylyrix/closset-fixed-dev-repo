...
class AIService {
  ...
  async getDesignSuggestions(currentDesign: any): Promise<AISuggestion[]> {
...
Return only valid JSON without any additional text.`;
  ...
  async getColorHarmony(baseColor: string): Promise<ColorHarmony[]> {
...
Return only valid JSON without any additional text.`;
  ...
  async generatePatternDescription(patternType: string, settings: any): Promise<string> {
...
Keep the response concise but informative (2-3 paragraphs).`;
  ...
  async optimizeDesignForPrint(designData: any): Promise<string[]> {
...
Return as a simple array of recommendation strings.`;
  ...
  private getFallbackTrends(): AITrendAnalysis[] {
    return [
      {
        id: 'trend_1',
        category: 'Color Trends',
  ...
  private getFallbackColorHarmony(baseColor: string): ColorHarmony[] {
    return [
      {
        id: 'harmony_1',
        type: 'complementary',
...
export type { AISuggestion, AITrendAnalysis, ColorHarmony, AIServiceResponse };
