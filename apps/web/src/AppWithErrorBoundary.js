...
export const AppWithErrorBoundary = () => {
    ...
    useEffect(() => {
        const handleError = (event) => {
            performanceMonitor.trackError(new Error(event.message), 'AppWithErrorBoundary', 'high', {
        ...
        const handleUnhandledRejection = (event) => {
            performanceMonitor.trackError(new Error(event.reason), 'AppWithErrorBoundary', 'high', {
    ...
    if (isLoading) {
        return (_jsx(LoadingOverlay, { isVisible: true, message: loadingMessage, progress: loadingProgress, showProgress: true, type: "loading" }));
    }
    return (_jsx(ErrorBoundary, { fallback: _jsxs("div", { style: {
                display: 'flex',
                ...
                fontFamily: 'system-ui, -apple-system, sans-serif'
            }, children: [_jsx("div", { style: { fontSize: '4rem', marginBottom: '1rem' }, children: "\uD83D\uDEA8" }), _jsx("h1", { style: { color: '#dc3545', marginBottom: '1rem' }, children: "Application Error" }), _jsx("p", { style: { color: '#6c757d', textAlign: 'center', maxWidth: '500px' }, children: "We're sorry, but something went wrong with the application. Please try refreshing the page or contact support if the problem persists." }), _jsx("button", { onClick: () => window.location.reload(), style: {
                        ...
                        fontWeight: '500',
                        marginTop: '1rem'
                    }, children: "Refresh Page" })] }), onError: (error, errorInfo) => {
            performanceMonitor.trackError(error, 'AppWithErrorBoundary', 'critical', {
                componentStack: errorInfo.componentStack,
                timestamp: Date.now()
            });
        }, showDetails: process.env.NODE_ENV === 'development', children: _jsx(Suspense, { fallback: _jsx(LoadingOverlay, { isVisible: true, message: "Loading application...", type: "loading" }), children: _jsx(App, {}) }) }));
};
