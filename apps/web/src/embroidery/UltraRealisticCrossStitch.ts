...
export interface CrossStitchGeometry {
...
export interface CrossStitchPattern {
  type: 'full' | 'half' | 'quarter' | 'three-quarter' | 'backstitch' | 'french-knot';
  direction: 'normal' | 'diagonal' | 'mixed';
  density: number; // Stitches per inch
  complexity: number; // 1-10, affects pattern detail
  symmetry: boolean; // Whether to maintain symmetry
}

export interface CrossStitchMaterial {
...
export interface CrossStitchLighting {
  ...
  rimLighting: boolean;
  rimIntensity: number;
  fabricShading: boolean; // Whether to shade based on fabric weave
}

export interface CrossStitch3DProperties {
  ...
  underlayType: 'none' | 'center' | 'contour' | 'grid';
  ...
  fabricWeave: 'aida' | 'evenweave' | 'linen' | 'canvas';
  fabricCount: number; // Threads per inch
}
...
export class UltraRealisticCrossStitch {
   ...
   */
  static generateUltraRealisticCrossStitch(
    geometry: CrossStitchGeometry,
    pattern: CrossStitchPattern,
    material: CrossStitchMaterial,
    lighting: CrossStitchLighting,
    properties3D: CrossStitch3DProperties
  ): AdvancedStitch[] {
    try {
      ...
      centralizedErrorHandler.handleError(
        error as Error,
        { component: 'UltraRealisticCrossStitch', function: 'generateUltraRealisticCrossStitch' },
        ErrorSeverity.HIGH,
   ...
   */
  private static generateBaseCrossStitchPattern(
    geometry: CrossStitchGeometry,
    pattern: CrossStitchPattern,
    properties3D: CrossStitch3DProperties
  ): any[] {
    ...
    for (let row = 0; row < geometry.grid.length; row++) {
      for (let col = 0; col < geometry.grid[row].length; col++) {
        ...
        const crossStitch = this.generateCrossStitchForCell(
          cell, 
          cellSize, 
          pattern, 
          properties3D,
   ...
   */
  private static generateCrossStitchForCell(
   ...
   */
  private static generateFullCrossStitch(
   ...
   */
  private static generateHalfCrossStitch(
    topLeft: { x: number; y: number; z: number },
    topRight: { x: number; y: number; z: number },
    bottomLeft: { x: number; y: number; z: number },
    bottomRight: { x: number; y: number; z: number },
   ...
   */
  private static generateQuarterCrossStitch(
    topLeft: { x: number; y: number; z: number },
    topRight: { x: number; y: number; z: number },
    bottomLeft: { x: number; y: number; z: number },
    bottomRight: { x: number; y: number; z: number },
   ...
   */
  private static generateThreeQuarterCrossStitch(
    topLeft: { x: number; y: number; z: number },
    topRight: { x: number; y: number; z: number },
    bottomLeft: { x: number; y: number; z: number },
    bottomRight: { x: number; y: number; z: number },
   ...
   */
  private static generateFrenchKnot(
    cell: { x: number; y: number; z: number },
    cellSize: number,
    pattern: CrossStitchPattern,
    properties3D: CrossStitch3DProperties
   ...
   */
  private static generateStitchLeg(
   ...
   */
  private static apply3DPadding(pattern: any[], properties3D: CrossStitch3DProperties): any[] {
  ...
  private static calculateFabricShading(point: any, properties3D: CrossStitch3DProperties): number {
  ...
  private static getFabricWeavePattern(weave: string, count: number): { frequency: number; amplitude: number } {
    switch (weave) {
      case 'aida':
  ...
  private static generateMaterialProperties(material: CrossStitchMaterial, stitch: any): any {
    return {
  ...
  private static calculateThreadColor(material: CrossStitchMaterial, stitch: any): string {
  ...
  private static generateNormalMap(material: CrossStitchMaterial): string {
  ...
  private static calculateShadowOffset(stitch: any, lighting: CrossStitchLighting): { x: number; y: number; z: number } {
    const lightDir = this.normalizeVector(lighting.lightDirection);
