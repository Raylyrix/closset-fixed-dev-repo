...
export interface StitchAppearance {
  heightMm: number; // thread height for relief
  widthMm: number; // thread width footprint
  twist: number; // twisting factor for specular anisotropy
  roughness: number; // for shader
  metallic: number; // for shader (usually 0)
  scattering: number; // subsurface-like scattering factor
}

export interface StitchBehavior {
  spacingMm: number; // distance between stitch points
  tension: number; // how tightly the stitch hugs the surface (0..1)
  density: number; // for fills (stitches per mm)
  randomness: number; // small irregularity to look real (0..1)
}
...
export interface StitchDefinition {
  ...
  behavior: StitchBehavior;
  // Rendering hints for future shader pipeline
  shading: {
    anisotropic: boolean;
    parallax: boolean;
...
export const StitchCatalog: Record<StitchType, StitchDefinition> = {
  running: {
    id: 'running',
    name: 'Running Stitch',
...
export const getStitchDefinition = (type: StitchType): StitchDefinition => StitchCatalog[type];
