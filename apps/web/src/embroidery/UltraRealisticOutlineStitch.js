...
export class UltraRealisticOutlineStitch {
     ...
     */
    static generateUltraRealisticOutline(geometry, pattern, material, lighting, properties3D) {
        try {
            ...
            centralizedErrorHandler.handleError(error, { component: 'UltraRealisticOutlineStitch', function: 'generateUltraRealisticOutline' }, ErrorSeverity.HIGH, ErrorCategory.EMBROIDERY);
            return [];
        }
    }
    /**
     ...
     */
    static generateBaseOutlinePattern(geometry, pattern, properties3D) {
        const patterns = [];
        switch (pattern.type) {
            case 'running':
     ...
     */
    static generateRunningStitch(geometry, pattern, properties3D) {
        const patterns = [];
     ...
     */
    static generateBackstitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateStemStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateSplitStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateChainStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateBlanketStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateFeatherStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
     ...
     */
    static generateHerringboneStitch(geometry, pattern, properties3D) {
        const patterns = [];
        const path = geometry.path;
        const spacing = 1.0 / pattern.density; // mm between stitches
        for (let i = 0; i < path.length - 1; i++) {
    ...
    static calculateEdgeHighlighting(point, properties3D) {
...
UltraRealisticOutlineStitch.PIXELS_PER_MM = 3.7795275591; // 96 DPI
UltraRealisticOutlineStitch.MAX_STITCH_LENGTH = 8.0; // mm
UltraRealisticOutlineStitch.MIN_STITCH_LENGTH = 0.5; // mm
