...
export async function uploadModel(file: File): Promise<{ id: string; url: string }> {
...
export async function saveProject(project: any): Promise<{ id: string }> {
...
export async function upscalePng(pngBlob: Blob, scale = 2): Promise<Blob> {
  const form = new FormData();
